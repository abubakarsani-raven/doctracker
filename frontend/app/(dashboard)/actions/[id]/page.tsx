...
import { ArrowLeft, CheckCircle2, Clock, User, Building2, FileText, Upload, MessageSquare, Workflow as WorkflowIcon } from "lucide-react";
...
export default function ActionDetailPage() {
  ...
  const loadAction = async () => {
    setLoading(true);
    try {
      const [allActions, workflowsData] = await Promise.all([
        api.getActions(),
        api.getWorkflows(),
      ]);
      ...
      // Merge and find action
      const allActionsList = [...allActions, ...localActions];
      const foundAction = allActionsList.find((a: any) => a.id === actionId);
      
      if (foundAction) {
        setAction(foundAction);
        
        // Load workflow if action belongs to one
        if (foundAction.workflowId) {
          const allWorkflows = [...workflowsData, ...localWorkflows];
          const foundWorkflow = allWorkflows.find((w: any) => w.id === foundAction.workflowId);
  ...
  const handleActionClick = () => {
    ...
    // Check if user can respond to this action
    const canRespond = canRespondToAction(action, workflow, currentUser);
    const isAssigned = isAssignedToAction(action, currentUser);
    
    // If user is workflow participant but not assigned, they cannot respond
    if (!canRespond && !isAssigned) {
      return; // Do nothing - user can only view
    }
    
    if (action.type === "document_upload" && action.status === "pending" && canRespond) {
      setUploadDialogOpen(true);
    } else if (action.type === "document_upload" && action.status === "document_uploaded" && canRespond) {
      setCompletionDialogOpen(true);
    } else if (action.type === "request_response" && !action.response && canRespond) {
      setResponseDialogOpen(true);
    } else if (action.type === "request_response" && action.status === "response_received") {
      // If user is requester and assigned, allow completion
  ...
  const handleActionComplete = async () => {
    await loadAction();
    // Update workflow progress if action is linked to a workflow
  ...
  const isOverdue = action.dueDate && action.status !== "completed" && new Date(action.dueDate) < new Date();
  const isRequester = currentUser && action.createdBy === currentUser.name;
  const canComplete = action.status !== "completed";
  
  // Check if user can respond and is assigned
  const canRespond = canRespondToAction(action, workflow, currentUser);
  const isAssigned = isAssignedToAction(action, currentUser);
  
  // Determine if user can actually complete the action
  const canActuallyComplete = canComplete && canRespond && isAssigned;
  
  // Check if user is view-only (part of workflow but not assigned)
  ...
  // Only show document link if documentId exists and is not empty
  const hasValidDocument = action.documentId && action.documentId.trim() !== "" && action.documentName;
    ...
    <div className="space-y-6">
          ...
          {canActuallyComplete && canRespond && (
            <Button onClick={handleActionClick}>
              ...
              {((action.type === "document_upload" && action.status === "document_uploaded") ||
                (action.type === "request_response" && action.status === "response_received" && isRequester && isAssigned) ||
                (action.type === "regular")) && (
                <>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                {isViewOnly && (
                  <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-950 rounded-md border border-yellow-200 dark:border-yellow-800">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                      <strong>View Only:</strong> You are part of this workflow but not assigned to this action. 
                      You can view the action but cannot respond or complete it.
                    </p>
                  </div>
                )}
                {isPendingApproval && (
                  <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-950 rounded-md border border-yellow-200 dark:border-yellow-800">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                      <strong>Pending Approval:</strong> This cross-company action is waiting for approval from the target company. 
                      You cannot complete this action until it has been approved.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            {action.type === "document_upload" && action.status === "document_uploaded" && action.uploadedDocumentName && (
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  <div className="flex items-center gap-3 p-3 rounded-lg border bg-green-50 dark:bg-green-950">
                    ...
                    <div className="flex-1">
                      <p className="font-medium">{action.uploadedDocumentName}</p>
                      <p className="text-sm text-muted-foreground">
                        Uploaded {action.uploadedAt && formatDistanceToNow(new Date(action.uploadedAt), { addSuffix: true })}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-md">
                    <p className="text-sm">{action.requestDetails || "No request details provided."}</p>
                  </div>
                  {action.response && (
                    <>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm">
                    {action.type === "document_upload" ? "Document Upload" :
                     action.type === "request_response" ? "Request/Response" :
                     "Regular Action"}
                  </p>
                </div>
                <div>
      ...
      {action && action.type === "document_upload" && canRespond && (
      ...
      {action && action.type === "request_response" && canRespond && (
        <RequestResponseActionDialog
          ...
          actionId={actionId}
          action={action}
          onResponseComplete={handleActionComplete}
